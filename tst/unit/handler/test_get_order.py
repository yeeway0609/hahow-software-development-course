...
def test_validate_input_when_succeed(test_app_context):
    with test_app_context():
        try:
            GetOrderHandler.validate_input({}) # 測試輸入空值
            GetOrderHandler.validate_input({"order_id": "test_order_id"}) # 測試輸入符合格式的值
...
def test_validate_input_when_failed(test_app_context):
    with test_app_context():
        with pytest.raises(InvalidAPIUsageException):
            GetOrderHandler.validate_input({"bad_field": ""}) # 測試輸入不存在的欄位
        with pytest.raises(InvalidAPIUsageException):
            GetOrderHandler.validate_input({"order_id": ""}) # 測試輸入空值
        with pytest.raises(InvalidAPIUsageException):
            GetOrderHandler.validate_input({"order_id": "bad_order_id", "extra": ""}) # 測試輸入多餘的欄位
